...
from Interaction import my_keyboard
...
def StartMouseEvent(event):
    ...
    print(handle)
    #双击左键，开始，进入游戏窗口
    if clicktime[-1]-clicktime[-2]<0.5 and handle!=my_keyboard.HANDLE:
        my_keyboard.HANDLE=event.Window            #游戏窗口句柄
        # 获取游戏窗口位置
        LEFT, TOP, RIGHT, BOTTOM = win32gui.GetWindowRect(my_keyboard.HANDLE)
        # 获取游戏窗口句柄的类名和标题
        title = win32gui.GetWindowText(my_keyboard.HANDLE)
        clsname = win32gui.GetClassName(my_keyboard.HANDLE)
        print(title, clsname)
        # 取消鼠标钩子
        #hm.UnhookMouse()
        ...
        #模拟游戏输入
        #game_base_action()
        #win32api.PostMessage( HANDLE,WM_KEYDOWN,VK_UP,1 )
        key_press(VK_UP)
        #开始进行神经网络的循环
        #赵士陆 2019.8.25  20：54
        startNetwork()
    ...
    # 双击左键，结束，停止游戏
    if clicktime[-1] - clicktime[-2] < 0.5 and my_keyboard.HANDLE==handle:
        #key_up('up_arrow')
        key_up(VK_UP)
        print("finally")
        my_keyboard.HANDLE=-1
    # 返回True代表将事件继续传给其他句柄，为False则停止传递，即被拦截
    return True
